...
| --- | --- | --- | --- | --- | --- | --- |
| Project identity, philosophy, and license boundary | D1, D5, D87, D143/D241, D263 | `SHAPE_DECIDED` | `pending` | planned | partial docs/tooling metadata | D263 license files are present; normative spec extraction still pending. |
| Core syntax and declarations | D8-D18, D41, D54, D78, D179 | `SHAPE_DECIDED` | `pending` | planned | inherited Austral parser only | Needs Kyokai spec extraction before frontend changes are mature. |
| Type system, ownership, borrows, and implicit completions | D6, D7a/D7b, D14, D34, D46, D87, D187, D238-D240 | `SHAPE_DECIDED` | `pending` | planned | inherited Austral checker only | D238-D240 pipeline must be implemented before ergonomic surface forms are trustworthy. |
| Error handling, contracts, defer, panic/TPOE | D2, D15/D15a, D24, D84, D89, D111, D124-D125, D246, D259-D260 | `SHAPE_DECIDED` | `pending` | planned | inherited Austral behavior only | Needs runtime-failure spec and conformance matrix. |
| FFI, unsafe, ABI, layout, and backend safety | D20/D242/D242a, D31, D80, D113a/D113b, D139, D245, D250-D251, D257 | `SHAPE_DECIDED` | `pending` | planned | inherited C backend only | Needs safe Kyokai boundary wrappers and UB-closure tests. |
| Concurrency, atomics, channels, poller, and signals | D3, D3a/D3b, D90-D95, D100-D101, D141, D146, D156, D164, D168, D183-D184, D212, D234-D237, D247-D248, D252, D256, D258 | `SHAPE_DECIDED` | `pending` | planned | planned | Requires memory-model spec before implementation claims. |
| Standard library foundation and admission policy | D44, D74, D77, D229-D232, D259-D260 | `SHAPE_DECIDED` | `pending` | planned | inherited Austral stdlib only | D229 admission records needed per stable module. |
| Toolchain, package manager, diagnostics, formatter, docs, releases | D25-D29, D51, D78, D83, D86, D105, D137, D155 | `SHAPE_DECIDED` | `pending` | planned | inherited Austral CLI only | Public workflow docs exist; normative toolchain spec pending. |
| Formal calculus and proof | D143/D241 | `SHAPE_DECIDED` | `pending` | not applicable | research note exists | `lambda_K` paper proof must be drafted before 1.0. |
