...
1. A type is subject to **linear checking** when it is classified in the
   `Linear` universe (including structurally linear types and instances of
   generics declared with universe `Type` that the compiler classifies as
   `Linear`—see [Declaring Types](#declaring-types) in the type system), or when
   manipulating values whose static type involves a type parameter of kind
   `Type` (those values follow linear rules until fully known).
