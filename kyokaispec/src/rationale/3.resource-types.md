...
> **Note:** Early examples use **pseudocode** (Java-like `String`, `void`, and
> freestanding signatures), not Austral surface syntax. Normative Austral syntax
> and types appear in the main chapters (for example `Span[Nat8, Static]` rather
> than a `String` type).
...
![A graph with three nodes labeled 'openFile', 'writeString', and 'close File'. There are four arrows: from 'openFile' to 'writeString', from 'openFile' to 'closeFile', from 'writeString' to itself, and from 'writeString' to 'closeFile'.](../austral.github.io/assets/spec/file-api.svg)
...
type belong to. For **value types**, the main split is the `Free` versus `Linear`
universes (Austral surface syntax also uses `Type` and `Region` markers on
declarations; see the type system chapter). So for example we can write:
