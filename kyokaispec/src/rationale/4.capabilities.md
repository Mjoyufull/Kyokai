...
The solution is [capability-based security][cap]. Code should be permissioned. To
...
module Files is
    ...
    function makePath(value: Span[Nat8, Static]): Path;
    function disposePath(path: Path): Unit;

    -- Reading and writing.
    generic [R: Region]
    function readFile(path: &[Path, R]): Span[Nat8, Static];

    generic [R: Region]
    function writeFile(path: &![Path, R], content: Span[Nat8, Static]): Unit;
end module.
```
...
let secrets: Span[Nat8, Static] := readFile(&p);
...
module Files is
    ...
    function append(path: Path, name: Span[Nat8, Static]): Path;
...
proving that the client has access to a more powerful capability. This
recursion has to end somewhere.

The root of the capability hierarchy is a value of type `RootCapability`. The
program entrypoint may take no parameters or a single `RootCapability` argument
(see the compiler’s `Entrypoint` module). For our
