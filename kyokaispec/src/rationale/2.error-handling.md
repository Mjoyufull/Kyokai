...
3. **Memory Allocation Failure:** Programs written in a functional style often
   ...
   `Option` type or equivalent, and this propagates up through every client of
   that function.

   Nevertheless, returning an `Option` type (or equivalent) on memory
...
4. **Failure Conditions**. These errors are recoverable, in the sense that we
   ...
   codes. Secondly, an appropriate type system lets us have e.g. `Option` or
